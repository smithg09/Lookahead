...
function fillPipeline(fields, pipeline, context, path = '') {
  Object.keys(fields).forEach((fieldName) => {
    ...
    const { alias, lookahead = {} } = field;

    // `lookup` performs a lookup stage
    if (lookahead.lookup) {
      let lookup;
      const { collection, localField, foreignField, preserveNull, conds, sort, limit } = lookahead.lookup;
    ...
    if (lookahead.compose) {
      pipeline.push({
        $addFields: {
          [`${path}${alias}`]: {
            $ifNull: [
              { $concat: lookahead.compose.map((str) => (str.startsWith('$')) ? `$${path}${str.slice(1, str.length)}` : str) },
              '$$REMOVE',
            ],
          }
        }
    ...
    if (lookahead.expr) {
      const e = JSON.parse(lookahead.expr.replace('@path.', path));
      pipeline.push({
        $addFields: {
    ...
    if ((lookahead.lookup || lookahead.compose || lookahead.expr) && path) {
